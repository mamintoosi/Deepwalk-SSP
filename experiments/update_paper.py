...
Generates a new version of the paper with all findings incorporated.
"""
...
with open(os.path.join(RESULTS_DIR, "exp_C_seed_stability.json")) as f:
    seed_stability = json.load(f)

with open(os.path.join(RESULTS_DIR, "exp_statistical_analysis.json")) as f:
    stat_analysis = json.load(f)

with open(os.path.join(RESULTS_DIR, "exp_A_hyperparameter_sensitivity.json")) as f:
    hyperparam = json.load(f)

with open(os.path.join(RESULTS_DIR, "exp_D_clustering_stability.json")) as f:
    clustering_stab = json.load(f)

with open(os.path.join(RESULTS_DIR, "exp_B_runtime.json")) as f:
...
new_highlights = r"""
\begin{highlights}
\item DeepWalk graph embeddings achieve a \textbf{294\% improvement} in Silhouette Score ($0.604$ vs $0.153$) over traditional enrollment-matrix representations, validated by Wilcoxon signed-rank tests ($p < 0.000001$ per course).
...
highlights_pattern = r'\\begin\{highlights\}.*?\\end\{highlights\}'
content = re.sub(highlights_pattern, new_highlights.strip(), content, flags=re.DOTALL)

# ============================================================
# 3. FIND AND UPDATE EXPERIMENTAL RESULTS SECTION
# ============================================================

# Find the experimental results section
exp_start = content.find(r"\section{Experimental Results}")
if exp_start == -1:
    print("ERROR: Could not find Experimental Results section")
    exit(1)
...
# Extract the experimental results section
old_exp_section = content[exp_start:conc_start]

# Create new experimental results section with all findings
new_exp_section = r"""
...
# Replace the experimental results section
...
# Replace conclusion
conc_pattern = r'\\section\{Conclusion\}.*?(?=\\section\{Declarations\})'
content = re.sub(conc_pattern, new_conclusion.strip() + "\n\n", content, flags=re.DOTALL)

# ============================================================
# 5. UPDATE REFERENCES (add new citations if needed)
# ============================================================
# The references are already inline in the thebibliography environment, so no changes needed there.
